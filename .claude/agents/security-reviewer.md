...
1. `scan_path(path="<변경된 파일 또는 실제 소스 루트>", profile="dev-quick", max_files=500)` 1회만 실행.
   ...
   **결과의 `profile_fallback` 필드가 `null`이 아니면**(요청한 프로필을 못 찾아 대체 실행됐다는 뜻 —
   `profile_fallback.requested`/`.applied`/`.reason`에 사유가 담김) 발견 결과를 코칭에 쓰지 않는다. 대신
   "가벼운 점검 프로필을 못 찾아 전체 룰셋으로 돌았어요(검증 미완료) — 체커 설치 상태를 확인해 주세요"로
   안내한다.
