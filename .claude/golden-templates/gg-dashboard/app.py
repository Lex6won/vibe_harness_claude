...
접근제어는 nginx 앞단 Keycloak(Streamlit 자체 인증 없음).
"""
import os
...
# 실제로는 SQLAlchemy 파라미터 바인딩으로 조회. 여기선 더미.
# from sqlalchemy import create_engine, text
# df = pd.read_sql(text("SELECT dept, rate FROM budget WHERE year=:y"), engine, params={"y": 2026})
df = pd.DataFrame({"부서": ["기획", "복지", "환경", "교통"], "집행률(%)": [72, 65, 88, 54]})
