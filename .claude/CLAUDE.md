...
4. **구현**: `golden-templates/`에서 Track에 맞는 템플릿을 골라 **그 안에서** 기능을 구현한다(아래 예방 레일 준수). 사용자와 결과 보며 반복.
   ...
   - 다만 초보자는 "언제 점검을 요청해야 하는지" 자체를 모른다. 방금 짠 코드가 인증·개인정보·SQL/DB·
     새 패키지·외부 API/CDN/LLM 호출·명령실행 중 하나를 건드렸으면, quick을 **자동 실행하지 말고** "방금
     ○○ 관련 코드를 짰는데 가볍게 점검해드릴까요?"로 먼저 물어본다. 실제 호출은 사용자가 그렇다고
     답했을 때만(토큰 비용은 여전히 사용자 동의 후에만 발생) — 사소한 UI 문구·스타일 변경에는 권하지 않는다.
